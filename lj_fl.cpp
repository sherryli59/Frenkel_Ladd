...
int main(int argc, char const *argv[]){
    ...
    SpringPotential spring_pot(lattice_site,alpha, beta, box,com_adjust);
    LJPotential lj_pot(sigma, epsilon, cutoff, beta, box);
    long double default_weight=1;
    long double lj_weight=1-lambda;
    MixedPotential<LJPotential,SpringPotential> pot(lj_pot,spring_pot,lj_weight,lambda,beta);
    ...
    for (int n = 0; n<niterations+nequil; ++n){
        ...
        if( (n>nequil) && ( ((n-nequil)%out_freq)==0 ) ){
            ...
            long double dU=spring_pot.compute_potential(frame)-lj_pot.compute_potential(frame);
